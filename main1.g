...
func Postparams(w http.ResponseWriter, r *http.Request) {
	if r.Method == "POST" {
		...
		w.Header().Set("Content-Type", "application/json")
		req.Id = rand.Intn(50)
		log.Printf("APP1 Respondendo com ID %d \n", req.Id)
		req.Type = "Response"
...
func PrintEnv(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		fmt.Println("IP GOAPP2", os.Getenv("IP_APP2"))
		//fmt.Fprintf(w, os.Getenv("BAR"))

		fmt.Println("APP1 pronta para responder")
		w.WriteHeader(http.StatusOK)
		json.NewEncoder(w).Encode("APP1 com sucesso api pronta para responder - IP GOAPP2" + os.Getenv("IP_APP2"))
...
func main() {
	if os.Getenv("IP_APP2") == "" {
		log.Println("Precisa ser declarado as variaveis IP_APP2 e PORT ou PORT = default 8080 !!!")
		os.Exit(1)
	}
	port := func() string {
		if os.Getenv("PORT") == "" {
	...
	fmt.Println("Iniciando aplicacao APP1 na porta " + port)
	r := mux.NewRouter()
	//r.HandleFunc("/")
