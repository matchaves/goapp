...
func PrintEnv(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		...
		step := fmt.Sprintf("Street %v", randomdata.Street())
		InfoLogger.Printf("Street %v", randomdata.Street())
		WarningLogger.Printf("Street %v", randomdata.Street())
